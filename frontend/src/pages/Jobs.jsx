  ...
  const fetchJobs = useCallback(
    async (signal) => {
      try {
        ...
        const data = await get(`/jobs/?${params.toString()}`, { signal });
        setJobs(data.jobs || []);
  ...
  const triggerJobScan = async () => {
    ...
    try {
      ...
      if (err.status === 429) {
        setError(
          err.message ||
            'Rate limit reached. Please wait before scanning again.'
        );
    ...
    >
      ...
      {loading ? (
        ...
        <div>
          ...
          >
            Showing {jobs.length} of {totalJobs} application
            {totalJobs !== 1 ? 's' : ''}
          </p>
