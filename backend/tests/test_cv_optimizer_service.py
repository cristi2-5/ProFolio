...
class TestCVOptimizerService:
    ...
    async def test_optimize_cv_for_job_success(
        ...
        cv_optimizer_service.cv_optimizer.optimize_cv_for_job.return_value = (
            optimized_cv_data
        )

        # Execute optimization
        result = await cv_optimizer_service.optimize_cv_for_job(
            user=sample_user, job=sample_job, db=mock_db
        )
    ...
    @pytest.mark.asyncio
    async def test_optimize_cv_no_resume(
        self, cv_optimizer_service, sample_user, sample_job
    ):
    ...
    async def test_generate_cover_letter_success(
        ...
        cv_optimizer_service.cv_optimizer.generate_cover_letter.return_value = (
            generated_letter
        )
    ...
    async def test_export_optimized_cv_pdf(
        ...
        result_pdf, result_filename = (
            await cv_optimizer_service.export_optimized_cv_pdf(
                user=sample_user, job_id=str(sample_user_job.job_id), db=mock_db
            )
        )

        # Verify PDF exporter was called
        mock_pdf_exporter.export_cv_to_pdf.assert_called_once_with(
            optimized_cv=sample_user_job.optimized_cv, user_name=sample_user.full_name
        )
    ...
    @pytest.mark.asyncio
    async def test_export_cv_pdf_no_optimized_cv(
        self, cv_optimizer_service, sample_user
    ):
        ...
        with pytest.raises(ValueError, match="No optimized CV found"):
            await cv_optimizer_service.export_optimized_cv_pdf(
                user=sample_user, job_id=str(uuid.uuid4()), db=mock_db
            )
    ...
    async def test_export_cover_letter_pdf(
        ...
        result_pdf, result_filename = (
            await cv_optimizer_service.export_cover_letter_pdf(
                user=sample_user, job=sample_job, db=mock_db
            )
        )
    ...
    async def test_get_optimization_suggestions(
        ...
        cv_optimizer_service.cv_optimizer.get_optimization_suggestions.return_value = (
            suggestions
        )
    ...
    @pytest.mark.asyncio
    async def test_get_user_optimized_materials(
        self, cv_optimizer_service, sample_user
    ):
    ...
    async def test_error_handling_with_rollback(
        ...
        cv_optimizer_service.cv_optimizer.optimize_cv_for_job.side_effect = Exception(
            "AI API error"
        )
    ...
    async def test_service_initialization(self):
        ...
        with patch(
            "app.services.cv_optimizer_service.CVOptimizerAgent"
        ) as mock_agent_class:
            mock_agent = Mock()
