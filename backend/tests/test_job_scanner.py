...
class TestBuildSearchQuery:
    ...
    def test_title_plus_top_three_keywords(self, agent, mock_preferences):
        """Only the first 3 keywords are appended to avoid over-restriction."""
        mock_preferences.keywords = ["fastapi", "postgresql", "docker", "aws", "kubernetes"]
        result = agent._build_search_query(mock_preferences)
        assert "Python Developer" in result
        assert "fastapi" in result
        assert "postgresql" in result
        assert "docker" in result
        assert "aws" not in result  # 4th keyword — should be trimmed
...
class TestProcessAndDeduplicateJob:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_by_content_hash_returns_none(self, agent, sample_adzuna_job):
        """A job with matching company+title+description_hash is rejected even with different URL."""
        existing = MagicMock(spec=ScrapedJob)

        # First call (URL check) returns None → not a URL dupe
        # Second call (content hash check) returns existing → is a content dupe
        db = AsyncMock(spec=AsyncSession)
        first_call = MagicMock(scalar_one_or_none=MagicMock(return_value=None))
        second_call = MagicMock(scalar_one_or_none=MagicMock(return_value=existing))
...
class TestScan:
    ...
    async def test_scan_returns_new_jobs(
        self, agent, mock_user, mock_preferences, sample_adzuna_job, make_adzuna_response
    ):
        ...
        pref_result = MagicMock(scalar_one_or_none=MagicMock(return_value=mock_preferences))
        ...
        with patch("app.agents.job_scanner.get_adzuna_client") as mock_client_factory:
            ...
            with patch.object(agent.job_service, "match_jobs_to_user", new_callable=AsyncMock) as mock_match:
    ...
    @pytest.mark.asyncio
    async def test_scan_no_preferences_returns_empty(self, agent, mock_user):
        """Returns empty list when user has no preferences set."""
        db = AsyncMock(spec=AsyncSession)
        db.get = AsyncMock(return_value=mock_user)
        db.execute = AsyncMock(return_value=MagicMock(scalar_one_or_none=MagicMock(return_value=None)))
        db.rollback = AsyncMock()
    ...
    async def test_scan_missing_api_credentials_returns_empty(
        ...
        db = AsyncMock(spec=AsyncSession)
        db.get = AsyncMock(return_value=mock_user)
        db.execute = AsyncMock(return_value=MagicMock(scalar_one_or_none=MagicMock(return_value=mock_preferences)))
        db.rollback = AsyncMock()

        with patch("app.agents.job_scanner.get_adzuna_client") as mock_factory:
            mock_api = AsyncMock()
            mock_api.search_jobs = AsyncMock(side_effect=ValueError("credentials not configured"))
    ...
    async def test_scan_adzuna_error_propagates(
        ...
        with patch("app.agents.job_scanner.get_adzuna_client") as mock_factory:
            mock_api = AsyncMock()
            mock_api.search_jobs = AsyncMock(side_effect=AdzunaAPIError("API down", 500))
            mock_factory.return_value = mock_api
    ...
    async def test_scan_individual_job_error_does_not_abort(
        ...
        bad_job = {"redirect_url": None, "title": None, "company": None, "description": None, "location": None}
        good_job = {
        ...
        db.execute = AsyncMock(side_effect=[
            ...
            no_dupe, no_dupe,  # for good_job deduplication
        ])
        db.add = MagicMock()
        db.flush = AsyncMock()
        db.commit = AsyncMock()
        ...
        with patch("app.agents.job_scanner.get_adzuna_client") as mock_factory:
            mock_api = AsyncMock()
            mock_api.search_jobs = AsyncMock(return_value=make_adzuna_response(jobs=[bad_job, good_job]))
            mock_factory.return_value = mock_api
...
class TestScanAllUsers:
    ...
    async def test_aggregates_total_counts(self, agent):
        ...
        db.execute = AsyncMock(
            return_value=MagicMock(
                scalars=MagicMock(return_value=MagicMock(all=MagicMock(return_value=users)))
            )
        )
...
class TestHashingIntegration:
    ...
    def test_case_insensitive_normalization(self):
        ...
        from app.utils.hashing import create_description_hash
        assert create_description_hash("Python Developer") == create_description_hash("python developer")

    def test_html_stripped_before_hashing(self):
        """HTML tags are stripped so <b>Python</b> == Python."""
        from app.utils.hashing import create_description_hash
        assert create_description_hash("<b>Python</b> developer") == create_description_hash("Python developer")
